<template>
    <section ref="abc" style="overflow-y: auto;">
		<el-button class="filter-item"   type="primary"  size="mini" style="margin: 0 0 10px 10px;"   @click="addUserInfo ">添加</el-button>
        	<el-table :data="lists" :max-height="heights" border highlight-current-row v-loading="listLoading" style="width: 100%;" stripe >
				...
				<el-table-column :prop="col.field" :label="col.title" v-for="(col, index) in cols" :key="index" align="center" >
					<template slot-scope="scope">
                            ...
                            <!-- <span v-if="col.field=='id'" v-show="scope.row.edit" >{{ scope.row[col.field] }}</span> -->
			...
			<el-form :model="AdduserForm" ref="AdduserForm">
				...
				<!-- <el-form-item label="ID：" prop="id" :label-width="formLabelWidth">
					<el-input v-model="AdduserForm.id" style="width:300px"></el-input>
				</el-form-item> -->
				<el-form-item label="CODE码：" prop=" phoneNotes" :label-width="formLabelWidth">
					<el-input v-model="AdduserForm.phoneNotes" style="width:300px"></el-input>
				</el-form-item>	
				<!-- <el-form-item label="代码注释：" prop="phoneNotes" :label-width="formLabelWidth">
					<el-select  v-model="AdduserForm.phoneNotes" placeholder="请选择" style="width:200px">
				...
				<!-- </el-form-item>	  -->
				<el-form-item label="代码名称：" prop="phoneCode" :label-width="formLabelWidth">
					<el-input v-model="AdduserForm.phoneCode"  style="width:300px"></el-input>
				</el-form-item>	
				<el-form-item label="代码注释：" prop="phoneName" :label-width="formLabelWidth">
					<el-input v-model="AdduserForm.phoneName"  style="width:300px"></el-input>
				</el-form-item>				
			</el-form>
...
<script>
    ...
    import { PhoneCodList,PhoneCodeDelete,PhoneCodeUpdate,PhoneCodeInsert } from "@/api/basedata";
    export default{
        data(){
            return{
				total: 0,
                ...
                cols:[                
					{title:'CODE',field:'phoneNotes'},
					{title:'代码名称',field:'phoneCode'}, 
					{title:'代码注释',field:'phoneName'},                  
				],
				AdduserForm:{					
					// id:"",
					phoneCode:"",
       ...
       CanCelAddUserInfo(){
		...
		choice(AdduserForm){			
 			let para ={
				...
				phoneCode:this.AdduserForm.phoneCode,
				phoneNotes:this.AdduserForm.phoneNotes,
                ...
                if(valid){
                  PhoneCodeInsert(para).then(res =>{ 
                    if(res.data.success){
                        ...
                        this.lists.unshift(
                        {
							// "id":this.$refs['AdduserForm'].model.id,
							"phoneCode":this.$refs['AdduserForm'].model.phoneCode,
							"phoneNotes":this.$refs['AdduserForm'].model.phoneNotes,
							"phoneName":this.$refs['AdduserForm'].model.phoneName                      	                         
						},	
