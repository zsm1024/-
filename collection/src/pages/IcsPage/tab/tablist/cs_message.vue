    ...
    <el-collapse v-model="activeNames">
        ...
        <template>
          ...
          >
            ...
            <el-table-column
              ...
              v-for="(baseinfo, index) in baseinfo"
              :key="index"
              align="center"
      ...
      <el-collapse-item name="2" title="客户电话信息" id="asd" style="position:relative">
        <el-button
          class="filter-item"
          type="primary"
          style=" position:absolute;top:3px;left:125px"
          @click="addUserInfos = true"
        >添加</el-button>
        <el-table
          id="CusInfo"
          :data="items.customerPhones"
          border
          :row-class-name="tableRowClassNameCustom"
        >
          <el-table-column label="操作" width="70">
            <template slot-scope="scope">
              <el-button
                type="danger"
                size="mini"
                v-if=" scope.row.infoSource!='CMS'"
                @click.native.prevent="deleteRow(scope.$index, scope.row,items.customerPhones)"
              >移除</el-button>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              ...
              <i
                v-if="scope.row.infoSource!='CMS'&&cols.field=='name'&&cols.field!='effectiveness'&&cols.field!='phoneNum'"
                class="ivu-icon ivu-icon-ios-phone-portrait"
                @click="ring1(scope.row.phone,scope.row)"
              ></i>
              <!-- " icon="ios-phone-portrait"  -->
              <Input
                v-if="scope.row.infoSource!='CMS'&&cols.field!='effectiveness'&&cols.field=='phoneType'&&cols.field!='phoneNum'"
                size="small"
                v-model="scope.row[cols.field]"
                class="inputInner"
                style="text-align:center"
              ...
              ></i>
              <i
                v-if="scope.row.infoSource=='CMS'&&cols.field=='phone'"
                class="ivu-icon ivu-icon-ios-phone-portrait"
                @click="ring(scope.row.phone,scope.row)"
              ></i>
              <el-select
              ...
              >
                <el-option
                  ...
                  :key="index"
                  :value="item.phoneNotes  +'('+ item.phoneCode + ')'"
                ></el-option>
              </el-select>
              <el-select
      ...
      <el-collapse-item name="8" title="备注" style="position:relative">
        <el-button
          type="primary"
          size="mini"
          v-on:click="remarkopenList"
          style="position:absolute;top:3px;left:70px"
        >编辑</el-button>
      ...
      <el-collapse-item name="3" title="客户地址信息" style="position:relative">
        <el-button
          ...
          style="  position:absolute;top:3px;left:125px"
          type="primary"
          @click="addWorkInfos = true"
        >添加</el-button>
        <el-table
          :data="items.customerAddresses"
          id="addressInfo"
          border
          :row-class-name="tableRowClassNameAddress"
        >
          <el-table-column label="操作" align="center" width="70">
            <template slot-scope="scope">
              ...
              <el-button
                ...
                size="mini"
                v-if=" scope.row.infoSource!='CMS'"
                @click.native.prevent="deleteAdress(scope.$index, scope.row,items.customerAddresses)"
              >移除</el-button>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              ...
              <Input
                v-if="scope.row.infoSource!='CMS'&&(cols1.field=='relationship'||cols1.field=='province'||cols1.field=='city'||cols1.field=='address'||cols1.field=='propertyType')"
                size="small"
    ...
    <el-dialog title="备注" :visible.sync="remarkopen" :show-close="false" id="remarkopen">
      <el-form :model="remarkform" ref="remarkform">
        <el-form-item label="备注内容" :label-width="formLabelWidth">
          <el-input type="textarea" :maxlength="2000" :value="marks" v-model="remarkform.remarks"></el-input>
          <span>(不超过2000字)</span>
        </el-form-item>
      </el-form>
      ...
      >
        <el-form-item label="角色：" prop="roleName" :label-width="formLabelWidth">
          <el-select v-model="AdduserForm.roleName" placeholder="角色" style="width:300px">
            <el-option v-for="item in roleTypes" :key="item.id" :label="item.val" :value="item.val"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="关系：" prop="relationship" :label-width="formLabelWidth">
          <el-select v-model="AdduserForm.relationship" placeholder="关系" style="width:300px">
            <el-option v-for="item in relations" :key="item.id" :label="item.val" :value="item.val"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="电话码：" prop="phoneNum" :label-width="formLabelWidth">
          <el-select v-model="AdduserForm.phoneNum" style="width:300px">
            <el-option
              v-for="(item,index) in PhoneCodeList"
              :key="index"
              :value="item.phoneCode +'('+ item.phoneNotes + ')'"
            ></el-option>
          </el-select>
        </el-form-item>
