...
export default {
  data() {
    return {
      ...
      cols: [
        ...
        { title: "预派案到期日", field: "deadTimeOfTheSendCase", width: "90" },
        { title: "委托金额", field: "entrustMoney", width: "60" },
        { title: "委托逾期天数", field: "entrustOverDueDays", width: "90" },
        { title: "委托逾期金额", field: "overdueReceivables", width: "90" },
  ...
  methods: {
    ...
    hostList(item) {
      ...
      window.open(this.exportPaths + "/os/osCaseExport?ids=" + ids);
    },
    hostListAll(item) {
      let para = {
      ...
      listAllExport(para).then(res => {
        ...
        window.open(
          this.exportPaths + "/os/osCaseExport?ids=" + this.idsAllexport
        );
      });
    },
