		...
		<el-button class="filter-item"   type="primary"  style="margin: 0 0 10px 10px;"  @click="toggleSelection(datas)">计算</el-button>
        	<el-table :data="lists" :max-height="heights" border highlight-current-row v-loading="listLoading" style="width: 100%;" stripe >
				...
				<el-table-column :prop="col.field" :label="col.title" v-for="(col, index) in cols" :key="index" align="center" >
					<template slot-scope="scope">
              <span v-show="scope.row.edit" v-if="(col.field!='depositDate'&&col.field!='updateTime')" >{{ scope.row[col.field] }}</span>
              ...
              <el-date-picker v-show="scope.row.edit" type="date" placeholder="选择日期" format='yyyy-MM-dd' value-format="timestamp" v-if="col.field=='depositDate'"  style="width: 100%;" v-model="scope.row[col.field]" @change="gettime(scope.row[col.field])"></el-date-picker>
						  <el-select class="comSty" v-show="scope.row.edit" v-if="col.field=='updateTime'" v-model="scope.row[col.field]" placeholder="请选择方式">
...
<script>
import { commission, delInfo$, updateInfo$, addInfo$ } from "@/api/basedata";
export default {
  data() {
    return {
      // total: 0,
