		...
		<el-table :data="lists" :max-height="heights" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;" stripe>
			...
			<el-table-column label="操作" width="120"  align="center" >
				<template  slot-scope="scope">
					 <router-link class="a-href" :to="{path:'/IcsPage/monitor/stributotaldetail/'+scope.row.id}">详情</router-link>
				</template>
			</el-table-column>
...
<script>
	...
	import { userstation } from '@/api/monitor';
	...
	export default {
		...
		methods: {
            addTab(index,row){
				...
				let component = resolve => require([`@/pages/IcsPage/monitor/${indexlink}`], resolve)
